...
async function seedUser () {
  try {
    ...
    userToBeInserted = {
      'password':'ewI2RstJx9RG5LT',
      'isDeleted':false,
      'username':'Kendrick_Sawayn48',
      'email':'Gene18@gmail.com',
      'isActive':true,
      'userType':authConstant.USER_TYPES.User
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let user = await dbService.updateOne(User, { 'username':'Kendrick_Sawayn48' }, userToBeInserted,  { upsert: true });
    userToBeInserted = {
      'password':'w9liInBqbRij9Yc',
      'isDeleted':false,
      'username':'Eladio.Johnson',
      'email':'Stephania20@hotmail.com',
      'isActive':true,
      'userType':authConstant.USER_TYPES.Admin
    };
    userToBeInserted.password = await  bcrypt.hash(userToBeInserted.password, 8);
    let admin = await dbService.updateOne(User, { 'username':'Eladio.Johnson' }, userToBeInserted,  { upsert: true });
